...
=======
	...
	static int		fd_old, fd_new, char_cnt, max, i;
	static int		spaces;
		...
		while (*p != 0x0a && (void *)p < (map + statb.st_size))
		  {
			if (*p == 0x20)
			  {
				line_buf[i++] = *p++;
				while (*p == 0x20 && (void *)p < (map + (statb.st_size-1)))
					++p;
			  }
			line_buf[i++] = *p++;
		  }
	...
	while (p < (char *)(map + statb.st_size))
	  {
		...
		while (char_cnt < MAX_LENGTH && p < (char *)(map + statb.st_size))
			{ ++char_cnt; ++p; }

		/* deal with new line characters; conserve paragraph structure */
		end = p; p = start; spaces &= ~spaces;
		while (p < end)
		  {
			if (*p == 0x0a && (*(p+1) == 0x0a || *(p-1) == 0x0a))
			  {
...
=======
	...
	if (!LENGTH) // find len of longest line and justify according to that
	  {
		MAX_LENGTH &= ~MAX_LENGTH; // au cas où
		while (p < (char *)(map + statb.st_size))
		  {
			char_cnt &= ~char_cnt;
			while (*p == 0x20 || *p == 0x09)
...
=======
	...
	static int		i;
	static unsigned		int_prog, int_prog_sv;

	len = strlen((char *)arg);
	int_prog &= ~int_prog;
	int_prog_sv &= ~int_prog_sv;
		...
		if (progress == 1)
		  {
			...
			right(to_print);
			printf("%s%3d%%\e[m\r\n", DISPLAY_COLOUR, (unsigned)int_prog);
			goto __show_progress_end;
		  }
		if ((progress*100) >= min)
		  {
			printf("%s%c\e[m", PROGRESS_COLOUR, 0x23);
			while (min < (progress*100))
				min += period;
			--to_print;
		  }
		if (int_prog_sv != int_prog)
		  {
			right(to_print);
			printf("%s%3d%%\e[m", DISPLAY_COLOUR, (unsigned)int_prog);
			left(to_print+3);
